<?php
/**
 * E-Condo Packages - Editar Village
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $result = $villageModel->update($villageId, $data);
        ...
        if ($result) {
            ...
            $logModel->insert([
                ...
                'entity_type' => 'village',
                'entity_id' => $villageId,
                'description' => "Village '{$name}' atualizada",
                'ip_address' => $_SERVER['REMOTE_ADDR'] ?? null,
                'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? null
            ]);
            
            setFlash('success', 'Village atualizada com sucesso!');
            redirect('/villages/list.php');
        } else {
            $errors[] = 'Erro ao atualizar village.';
        }
    }
} else {
    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h3">
                    <i class="bi bi-pencil"></i> Editar Village
                </h1>
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary btn-lg">
                                <i class="bi bi-check-circle"></i> Atualizar Village
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <div class="col-lg-4">
            <div class="card border-info">
                <div class="card-header bg-info text-white">
                    <h5 class="mb-0"><i class="bi bi-house"></i> Casas desta Village</h5>
                </div>
                <div class="card-body">
                    <?php
                    ...
                    <p><strong>Total de casas:</strong> <?= count($houses) ?></p>
                    
                    <?php if (count($houses) > 0): ?>
                        <div style="max-height: 300px; overflow-y: auto;">
                            <?php foreach ($houses as $house): ?>
                                <div class="border-bottom py-2">
                                    <i class="bi bi-house"></i> Casa <?= htmlspecialchars($house['house_number']) ?>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    <?php else: ?>
                        <p class="text-muted">Nenhuma casa cadastrada ainda.</p>
                    <?php endif; ?>
                    
                    <a href="<?= SITE_URL ?>/houses/create.php?village_id=<?= $villageId ?>" class="btn btn-sm btn-primary mt-3 w-100">
                        <i class="bi bi-plus-circle"></i> Adicionar Casa
                    </a>
                </div>
            </div>
        </div>
